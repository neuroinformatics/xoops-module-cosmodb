<?php
	...
	include 'style.css';
	$lim = 10; $kw = ''; $andor = 0; $method='';
	if(isset($_GET['kw']) && !isset($_GET['st'])){
		$method = 'search';
		$andor = intval($_GET['andor']);
		...
		case 'search':
			...
			if($xoopsDB->getRowsNum($rs)){
				while($row = $xoopsDB->fetchArray($rs)){
					if(!in_array($row['com_id'], $list)) $list[] = $row['com_id'];
				}
			}
	...
	function setResult($list, $kw){
		...
		foreach($list as $v){
			...
			if(strlen($message) > 200) $message = mbsubstr($message, 0, 200);
