...
class listmanager
{
    ...
    public function changeSize($name)
    {
        ...
        $size = array();
        for ($i = 0, $iMax = count($this->thumb_size); $i < $iMax; ++$i) {
     ...
     * @return string $thumb_link (thumbnail size selection box)
     */
    ...
    {
        $this->sort_target = (int)$sort_target;
     ...
     * @return string $sortbox (sort box)
     */
    ...
    public function setTextbox($comp_id, $target, $system = 0)
    {
        ...
        $t2     = explode(' ', $target);
        for ($i = 0, $iMax = count($t2); $i < $iMax; ++$i) {
            if ($t != '') {
                $t .= ' OR ';
            }
            $t .= "value like '%" . $t2[$i] . "%'";
     ...
     *
     * making refine box
     * @return string $value (refine box)
     */
        ...
        while ($row = $this->db->fetchArray($rs)) {
            ...
            switch ($row['type']) {
                ...
                case '1':
                    if ($row['name'] === 'Author') {
                        ...
                        for ($i = 1; $i < 3; ++$i) {
                            ...
                            for ($j = 0, $jMax = count($year); $j < $jMax; ++$j) {
                ...
                case '2':
                    ...
                    for ($j = 0, $jMax = count($svalue); $j < $jMax; ++$j) {
    ...
    public function setRefine($author, $from, $to, $component, $mode, $refineby)
    {
        ...
        switch ($refineby) {
            ...
            case 3:
                $this->refine = array();
                if (count($tmp2)) {
                    for ($i = 0, $iMax = count($tmp1); $i < $iMax; ++$i) {
        ...
        if ($mode == 1) {
            ...
            $new = array();
            for ($i = 0, $iMax = count($this->refine); $i < $iMax; ++$i) {
    ...
    public function setKLabels($kw, $andor, $notkws)
    {
        ...
        $notnum = array();
        $notkw  = '';
        for ($i = 0, $iMax = count($notkws); $i < $iMax; ++$i) {
        ...
        $notkws = '';
        for ($i = 0, $iMax = count($notkw); $i < $iMax; ++$i) {
            if (empty($notkw[$i])) {
                continue;
            }
            if (!empty($notkws)) {
     ...
     *
     * @param int $mode
     * @return array $return_list (label array)
     */
    public function getLabels($mode = 0)
    {
        ...
        if ($this->refine_flg || $this->tsearch_flg) {
            $label_list2 = array();
            for ($i = 0, $iMax = count($label_list); $i < $iMax; ++$i) {
                if (in_array($label_list[$i], $this->refine)) {
                    $label_list2[] = $label_list[$i];
                }
            }
            $label_list = array();
            for ($i = 0, $iMax = count($label_list2); $i < $iMax; ++$i) {
