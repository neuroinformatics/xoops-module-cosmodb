...
*/
	...
	function Change_component($url){
		...
		# do edit
			...
			$textmax=0;
			$onoff_refine=0;
			if(isset($_POST['comp_nonull'])) $nonull = intval($_POST['comp_nonull']);
			if(isset($_POST['comp_textmax'])) $textmax = intval($_POST['comp_textmax']);
			if(isset($_POST['comp_onoff_refine'])) $onoff_refine = intval($_POST['comp_onoff_refine']);
	...
	function Add_new_component($url){
		...
		if(isset($_POST['comp_name'])){
			...
			$default = $myts->stripSlashesGPC($_POST['comp_default']);
			$onoff = intval($_POST['comp_onoff']);
			$onoff_refine = intval($_POST['comp_onoff_refine']);			
			$sort = intval($_POST['comp_sort']);
	...
	function showForm($url, $act, $id){
		...
		$type = array('radio', 'checkbox', 'text', 'select');
		for($i=0; $i<4; $i++){
