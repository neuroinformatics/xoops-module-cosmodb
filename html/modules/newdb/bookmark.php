<?php
    ...
    if (isset($_GET['bd'])) {
        $bd_id = intval($_GET['bd']);
    } elseif (isset($_POST['bd'])) {
        $bd_id = intval($_POST['bd']);
    }
    if (empty($bd_id)) {
    ...
    switch ($mode) {
        ...
        case 'bdedit2':
            ...
            // update
                ...
                $dir_name4sql = addslashes($myts->stripSlashesGPC($_POST['dir_name']));
                $sort = intval($_POST['sort']);
                ($_POST['newbd'] == 'no') ? $pbd = '' : $pbd = ",pbd_id='".intval($_POST['newbd'])."'";
