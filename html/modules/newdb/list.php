...
include __DIR__ . '/class/listmanager.php';
...
if (isset($_GET['tsearch'])) {
    ...
    if (isset($_GET['all'])) {
        ...
        if (isset($_GET['year1']) && isset($_GET['year2'])) {
            $from = strtotime((int)$_GET['year1'] . '/' . (int)$_GET['month1'] . '/' . (int)$_GET['day1']);
            $to   = strtotime((int)$_GET['year2'] . '/' . (int)$_GET['month2'] . '/' . (int)$_GET['day2']);
        }
...
switch ($lm->type) {
    ...
    case '2':
        ...
        for ($i = 0, $iMax = count($label_list); $i < $iMax; ++$i) {
            ...
            $rs         = $xoopsDB->query($sql);
            $row        = $xoopsDB->fetchArray($rs);
            $thumb_temp = $lm->getValues($row['label_id'], $row['label'], $row['author'], $row['reg_date'], $row['views'],
                                         $xoopsModuleConfig['dname_flg']);
